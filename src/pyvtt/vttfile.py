...
from pyvtt.vttitem import VTTItem
...
class VTTFile(UserList, object):
    """
    VTT file descriptor.

    Provide a pure Python mapping on all metadata.

    VTTFile(items, eol, path, encoding)

    items -> list of VTTItem. Default to [].
    ...
    def from_string(cls, source, **kwargs):
        """
        from_string(source, **kwargs) -> VTTFile
    ...
    def stream(cls, source_file, error_handling=ERROR_PASS):
        ...
        This method yields VTTItem instances a soon as they have been parsed
        ...
        for index, line in enumerate(chain(source_file, '\n')):
            if line.strip() and line != "WEBVTT\n":
                ...
                if source and all(source):
                    try:
                        yield VTTItem.from_lines(source)
    ...
    def write_into(self, output_file, eol=None):
        ...
        for item in self:
            ...
            # It was kept in the VTTItem's text before but it really
